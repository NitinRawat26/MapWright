...
public sealed class PlaybookStore(MapWrightDatabase database)
{
    ...
    public IReadOnlyList<string> Import(IReadOnlyList<Playbook> playbooks, string actor)
    {
        ...
        var added = playbooks.Where(p => !existing.Any(e => e.Reference == p.Reference)).ToList();
        RequireValid(PlaybookValidator.Validate([.. existing, .. added]), "Imported playbooks");

        foreach (var playbook in added)
        {
            Insert(connection, transaction, playbook, actor);
            Log(connection, transaction, playbook.Id, playbook.Version, actor, "imported", null, playbook.Status, null);
        }

        transaction.Commit();
        return [.. added.Select(p => p.Reference)];
    }

    public Playbook Create(Playbook playbook, string actor)
    {
    ...
    public Playbook UpdateDraft(string id, string version, Playbook playbook, string actor)
    {
        ...
        Update(connection, transaction, updated, actor);
    ...
    public Playbook DraftNewVersion(string id, string fromVersion, string? newVersion, string actor, string? note)
    {
        using var connection = database.Open();
        using var transaction = connection.BeginTransaction();
        var source = Find(connection, transaction, id, fromVersion) ?? throw StoreException.NotFound($"Playbook '{id}@{fromVersion}'");
        ...
        Insert(connection, transaction, draft, actor);
    ...
    public Playbook Transition(string id, string version, PlaybookStatus to, string actor, string? note)
    {
        ...
        if (to == PlaybookStatus.Published)
        {
            ...
            {
                Update(connection, transaction, previous with { Status = PlaybookStatus.Retired }, actor);
        ...
        Update(connection, transaction, next, actor);
    ...
    private static Playbook? Find(SqliteConnection connection, SqliteTransaction? transaction, string id, string version)
    {
        using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = "SELECT json FROM playbook_versions WHERE id = $id AND version = $version";
        command.Parameters.AddWithValue("$id", id);
        command.Parameters.AddWithValue("$version", version);
        return command.ExecuteScalar() is string json ? PlaybookSerializer.Deserialize(json) : null;
    }
    ...
    private void Insert(SqliteConnection connection, SqliteTransaction transaction, Playbook playbook, string actor)
    {
        using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = """
            INSERT INTO playbook_versions (id, version, kind, name, status, owner, json, created_at, created_by, updated_at, updated_by)
            VALUES ($id, $version, $kind, $name, $status, $owner, $json, $now, $actor, $now, $actor)
            """;
        Bind(command, playbook, actor);
        command.ExecuteNonQuery();
    }

    private void Update(SqliteConnection connection, SqliteTransaction transaction, Playbook playbook, string actor)
    {
        using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = """
    ...
    private void Bind(SqliteCommand command, Playbook playbook, string actor)
    {
